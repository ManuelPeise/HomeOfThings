...
const RegisterComponent: React.FC<IProps> = (props) =>{
    ...
    const {model, isValidModel, handleReset, handleTextChange} = useFormModel<IRegisterData>(registerModel, validationCallback)
    ...
    const onRegister = React.useCallback(async() =>{
        const response = await apiService.postWithContent({requestOptions:{method: "POST", mode:"cors", body: JSON.stringify(model)}})
    ...
    return (<Paper elevation={4} component="div" style={{padding:"1rem", maxHeight: "60%"}}>
        <Title title="Registrierung" variant="h5" padding="2"/>
